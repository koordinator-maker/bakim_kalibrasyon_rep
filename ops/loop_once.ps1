# ops/loop_once.ps1  (Windows PowerShell 5.1) - Incremental Test Plan
...
# --- YENİ: Test akışı kontrolü ---
if (-not (Get-Variable -Name RunPlan           -ErrorAction SilentlyContinue)) { $RunPlan = 'auto' }       # auto|subset|full
if (-not (Get-Variable -Name FullAfterPass     -ErrorAction SilentlyContinue)) { $FullAfterPass = $true }  # subset PASS ise full
...
function Write-Utf8([string]$Path,[string]$Content){
  ...
  if (-not [IO.Path]::IsPathRooted($full)) {
    $full = Join-Path -Path ((Get-Location).Path) -ChildPath $full
  }
  try {
    $full = [IO.Path]::GetFullPath($full)
  } catch {
    throw ("Write-Utf8 GetFullPath hata: " + $full + " — " + # ops/loop_once.ps1  (Windows PowerShell 5.1) - Incremental Test Plan
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
trap { Write-Error $_; try { Stop-Transcript | Out-Null } catch {}; return }
...
function Write-Utf8([string]$Path,[string]$Content){
  $Utf8NoBom = [Text.UTF8Encoding]::new($false)
  if ([string]::IsNullOrWhiteSpace($Path)) { throw "Write-Utf8: empty path" }
  # Mutlak yolu güvenli oluştur
  $full = $Path
  if (-not [IO.Path]::IsPathRooted($full)) {
    $full = Join-Path -Path ((Get-Location).Path) -ChildPath $full
  }
  try { $full = [IO.Path]::GetFullPath($full) } catch { throw ("Write-Utf8 GetFullPath hata: " + $full + " — " + # ops/loop_once.ps1  (Windows PowerShell 5.1) - Incremental Test Plan
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
trap { Write-Error $_; try { Stop-Transcript | Out-Null } catch {}; return }
...
function Read-CsvStrict([string]$Path){
  ...
  $rows = @(Import-Csv -LiteralPath $Path)   # DİZİ garantisi
  if($rows.Length -eq 0){ throw "CSV satır yok: $Path" }
  $first = $rows[0]
  $need = @('id','title','severity','area','evidence','timestamp')
  foreach($k in $need){
    if(-not ($first.PSObject.Properties.Name -contains $k)){ throw "Eksik sütun: $k" }
  }
  return $rows
}
...
# ---- 5) TESTLER: önce subset, PASS olursa full ----
# Görev → dosya eşlemesi (yalnız mevcut testler)
...
try{
  ...
  if ((@($subset)).Length -gt 0){
    Invoke-Playwright -Files $subset -Summary ([ref]$testSummary)
    $testSummary.note = "subset:" + ((@($subset)) -join ', ')
    if ($FullAfterPass -and $testSummary.exitCode -eq 0){
      Invoke-Playwright -Files @() -Summary ([ref]$testSummary)  # FULL
      $testSummary.note += "; then full"
    }
  } else {
    Invoke-Playwright -Files @() -Summary ([ref]$testSummary)    # FULL
    $testSummary.note = "full"
  }
...
if($testSummary.invoked -and $testSummary.exitCode -eq 0){
  Update-TodoStatus -Csv $CsvPath -Id $task.id -NewStatus "done" -Note ("OK; " + $testSummary.note + "; bundle: " + (Split-Path $zipPath -Leaf))
