...
function Invoke-AiPoll {
  param(
    ...
    [Parameter(Mandatory=$true)][string]$StatusUrlOrTemplate,
    [int]$MaxSec = $( [int](Get-Env 'AI_POLL_MAX_SEC' '600') ),
    [int]$EverySec = $( [int](Get-Env 'AI_POLL_INTERVAL_SEC' '10') )
  )
  $cli = New-HttpClient -TimeoutMin ([Math]::Max(1,[int][Math]::Ceiling($EverySec/60.0)))
  try{
    $t0 = Get-Date
    while($true){
      $url = $StatusUrlOrTemplate
      if($url -match '\{id\}'){ $url = $url -replace '\{id\}', [regex]::Escape($Id) }
