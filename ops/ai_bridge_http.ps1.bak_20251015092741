...
function New-HttpClient {
  ...
  if ($proxyUrl) {
    try {
      ...
      $h.Proxy = $wp; $h.UseProxy = $true
    } catch { Write-Host "[ai-http] proxy parse hata: $proxyUrl - $($_.Exception.Message)" }
  } else {
    $h.UseProxy = $useProxy
  }
  # TLS doğrulamasını (geçici) kapat (SADECE teşhis için)
  if ($env:AI_TLS_INSECURE -and $env:AI_TLS_INSECURE.ToLower() -eq 'true') {
    try { $h.ServerCertificateCustomValidationCallback = { param($m,$c,$ch,$e) $true } }
    catch { Write-Host "[ai-http] TLS insecure ayarlanamadı: $($_.Exception.Message)" }
  }
  $client = New-Object System.Net.Http.HttpClient($h)
  $tm = ($TimeoutMin -as [double]); if(-not $tm -or $tm -le 0){ $tm = 1 }
  $client.Timeout = [TimeSpan]::FromMinutes($tm)
  return $client
}
function Test-AiEndpointQuick {
  param([string]$Url, [int]$TimeoutSec = 12)
  $o = [ordered]@{ Ok=$false; Code=0; Body=''; Error=''; Proxy='' }
  if ([string]::IsNullOrWhiteSpace($Url) -or ($Url -match '<GERCEK|YOUR-REAL|ENDPOINT>')) { $o.Error = "AI_ENDPOINT placeholder/boş: '$Url'"; return [pscustomobject]$o }
  try {
    $cli = New-HttpClient -TimeoutMin ([Math]::Max(1,[int][Math]::Ceiling($TimeoutSec/60.0)))
    $req = New-Object System.Net.Http.HttpRequestMessage([System.Net.Http.HttpMethod]::Get, $Url)
    $res = $cli.SendAsync($req).GetAwaiter().GetResult()
    $o.Code = [int]$res.StatusCode
    $o.Ok   = ($o.Code -ge 200 -and $o.Code -lt 500)
    $body = $res.Content.ReadAsStringAsync().GetAwaiter().GetResult()
    if ($body) { $max=[Math]::Min(500,$body.Length); $o.Body = $body.Substring(0,$max) }
  } catch { $o.Error = param(
  [string]$TaskId = "UH001",
  [string]$RequestsDir = ".",
  [string]$OutDir = "_otokodlama\\inbox"
...
function Invoke-AiSubmit {
  ...
  $http = New-HttpClient -TimeoutMin $TimeoutMin
  try {
...
if ($env:AI_POLL_MODE -and $env:AI_POLL_MODE.ToLower() -eq 'skip') { Write-Host '[ai-http] poll skipped (AI_POLL_MODE=skip) - sanity'; return }
if (-not $uploadOk) {
...
elseif ($resp -and $resp.token) { $id = $resp.token }
  ...
  # Fallback: yanitta id yok → sentetik id ile devam
  $id = "mock-" + ([Guid]::NewGuid().ToString("N"))
  Write-Host "[ai-http] submit OK fakat id yok; fallback id = $id"
}
