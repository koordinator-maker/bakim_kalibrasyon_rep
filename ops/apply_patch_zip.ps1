...
function Get-Prop($o,[string]$n,$d=$null){ if($null -eq $o){return $d}; $p=$o.PSObject.Properties[$n]; if($p){$p.Value}else{$d} }
function Resolve-Zip([string]$p){
  if ([string]::IsNullOrWhiteSpace($p)) { return $null }
  if ($p -like "*`**") {
    $parent = Split-Path $p -Parent; $leaf = Split-Path $p -Leaf
    return (Get-ChildItem -Path $parent -Filter $leaf -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1)
  }
  if (Test-Path $p) { return Get-Item $p }
  return $null
}
$zipItem = Resolve-Zip $ZipPath; if (-not $zipItem) { throw "Zip bulunamadi: $ZipPath" }
...
if ($manFile) { $manifest = Get-Content $manFile.FullName -Raw | ConvertFrom-Json }
else {
  ...
  $items = @(); foreach($f in $files){ $rel = $f.FullName -replace [regex]::Escape($tmp + [IO.Path]::DirectorySeparatorChar), ""; $rel = $rel -replace "\\","/"; $items += @{ path=$rel; action="modify" } }
...
$items = @(Get-Prop $manifest 'items' @()); if ($items.Count -gt $MaxFiles) { throw "Dosya sayisi limit disi" }
...
function Resolve-Safe([string]$Rel){
  $absRepo = (Resolve-Path $repo).Path; $full = [IO.Path]::GetFullPath((Join-Path $absRepo $Rel))
  if ($full -notlike "$absRepo*") { throw "Path traversal: $Rel" }; return $full
}
function Hash256([string]$p){ if(Test-Path $p){ try { (Get-FileHash -LiteralPath $p -Algorithm SHA256).Hash.ToLower() } catch { $null } } else { $null } }
function WriteText([string]$src,[string]$dst){ $dir = Split-Path -Parent $dst; if($dir){ New-Item -ItemType Directory -Force -Path $dir | Out-Null }; $content = Get-Content -LiteralPath $src -Raw; [IO.File]::WriteAllText($dst,$content,$UTF8) }
$plan = New-Object System.Collections.Generic.List[string]; $failed = $false
foreach($it in $items){
  $rel = Get-Prop $it 'path' $null; if([string]::IsNullOrWhiteSpace($rel)){ $failed=$true; $plan.Add("ERR path empty"); continue }
  ...
  $target = Resolve-Safe $rel; $src = Join-Path $tmp $rel
  $action = (""+(Get-Prop $it 'action' 'modify')).ToLower(); $before = Get-Prop $it 'sha256_before' $null
  switch ($action) {
    "delete" { if(-not $AllowDelete){ $failed=$true; $plan.Add("ERR delete forbidden: " + $rel); break }
    ...
    "create" { if(-not (Test-Path $src)){ $failed=$true; $plan.Add("ERR source missing: " + $rel); break }
    ...
    default  { if(-not (Test-Path $src)){ $failed=$true; $plan.Add("ERR source missing: " + $rel); break }
               if($before){ $cur = Hash256 $target; if($cur -and ($cur -ne $before.ToLower())){ $failed=$true; $plan.Add("ERR sha256_before mismatch: " + $rel); break } }
...
[IO.File]::WriteAllLines($log,$plan,$UTF8); Write-Host "[apply] log:" $log
