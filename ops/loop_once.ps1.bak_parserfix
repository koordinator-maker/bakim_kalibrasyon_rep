...
try{
  ...
  if ((@($subset)).Length -gt 0){
    ...
    if ($FullAfterPass -and $testSummary.exitCode -eq 0){
      ...
      $testSummary.note = ($testSummary.note + "; then full")
    }
  } else {
    Invoke-Playwright -Files @() -Summary ([ref]$testSummary)    # FULL
...
$det = Get-ChildItem "_otokodlama\reports" -Recurse -File -Filter "details_*.csv" -EA SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
$tot = Get-ChildItem "_otokodlama\reports" -Recurse -File -Filter "totals_*.csv"  -EA SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if($det){ $bundleItems += $det.FullName }
if($tot){ $bundleItems += $tot.FullName }
...
function Update-TodoStatus([string]$Csv,[string]$Id,[string]$NewStatus,[string]$Note){
  ...
  if($hit){
    $bak = ($Csv + ".bak_" + (Get-Date -Format yyyyMMddHHmmss))
