...
class MaintenanceAdminSite(AdminSite):
    ...
    # (Eski helperlar kalsın ama URL bağlamıyoruz)
    ...
    def checklist_create(self, request):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                freq = form.cleaned_data["bakim_kontrol_periyodu"]
                desc = form.cleaned_data["bakim_kontrol_tanimi"].strip()
    ...
    # -----------------------------
    # URLs (helper equipment rotalarını KALDIRDIK)
    # -----------------------------
    def get_urls(self):
        return [
...
'@
[IO.File]::WriteAllText($ADM, $PY, [Text.UTF8Encoding]::new($false))

# Güvenlik: Python syntax kontrolü
python -m py_compile $ADM
